...
%         if ((feature(1,1,j) - muhat_sub1_trn1)^2 + (feature(1,2,j) - muhat_sub1_trn2)^2) >= thr^2
    ...
    %         if ((feature(2,1,j) - muhat_sub1_trn1)^2 + (feature(2,2,j) - muhat_sub1_trn2)^2) < thr^2
            if (feature(k,1,j) < muhat_sub1_trn1 + thr) && (feature(k,1,j) > muhat_sub1_trn1 - thr) && (feature(k,2,j) < muhat_sub1_trn2 + thr) && (feature(k,2,j) > muhat_sub1_trn2 - thr)
                FA = FA + 1;
            end
        end
    end
...
for i = 1:changing_seed
    ...
    for j = 1:100
        rx = muhat_w_1 - 10*max(sigmahat_w_1, sigmahat_w_2) + (20*max(sigmahat_w_1, sigmahat_w_2)).*rand;
        ry = muhat_w_2 - 10*max(sigmahat_w_1, sigmahat_w_2) + (20*max(sigmahat_w_1, sigmahat_w_2)).*rand;
